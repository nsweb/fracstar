...
template <typename type> struct UniformVar;

static void RegisterVar( UniformVar<float>* v )
{
...
{
public:
    UniformVar( type val, const char* sz_name ) : m_var(val), m_name(sz_name)
    {
        RegisterVar(this);
    }
    operator type & ()
...
#define UNIFORM_LEVEL( type, lvl_name, value )     UniformVar<type> lvl_name = { value, #lvl_name };
...
void LevelShader::Create( Entity* owner, class json::Object* proto )
{
...
bool LevelShader::LoadShaders()
{
    ...
    if( !shader_file.Open( vs_name.c_str(), false ) )
        return false;
    
    shader_file.SerializeString( vs_src );
    shader_file.Close();
    
    if( !shader_file.Open( fs_name.c_str(), false ) )
        return false;
	...
	if( !shader_file.Open( cs_name.c_str(), false) )
		return false;
...
void LevelShader::CreateUniformVariables()
{
...
void LevelShader::AddToWorld()
{
    
	Super::AddToWorld();
}

void LevelShader::RemoveFromWorld()
{
...
void LevelShader::InterpAndSetUniforms( float time )
{
